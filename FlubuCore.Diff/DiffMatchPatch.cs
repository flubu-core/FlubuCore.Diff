    ...
    public class diff_match_patch
    {
            ...
            Dictionary<string, int> lineHash, int maxLines)
        {
            ...
            while (lineEnd < text.Length - 1)
            {
                ...
                line = JavaSubstring(text, lineStart, lineEnd + 1);
        ...
        public List<T> Splice<T>(List<T> input, int start, int count,
            params T[] objects)
        {
        ...
        public string JavaSubstring(string s, int begin, int end)
        {
                ...
                switch (diffs[pointer].operation)
                {
                    ...
                    case Operation.EQUAL:
                        ...
                        if (count_delete + count_insert > 1)
                        {
                            ...
                            if (count_delete == 0)
                            {
                                Splice(diffs, pointer - count_insert,
                                    count_delete + count_insert,
                                    new Diff(Operation.INSERT, text_insert));
                            }
                            else if (count_insert == 0)
                            {
                                Splice(diffs, pointer - count_delete,
                                    count_delete + count_insert,
        ...
        protected void patch_addContext(Patch patch, string text)
        {
                   ...
                   && pattern.Length < Match_MaxBits - Patch_Margin - Patch_Margin)
            {
                ...
                pattern = JavaSubstring(text, Math.Max(0, patch.start2 - padding),
            ...
            // Add the prefix.
            string prefix = JavaSubstring(text, Math.Max(0, patch.start2 - padding),
                patch.start2);
            if (prefix.Length != 0)
            {
            ...
            // Add the suffix.
            string suffix = JavaSubstring(text, patch.start2 + patch.length1,
                Math.Min(text.Length, patch.start2 + patch.length1 + padding));
            if (suffix.Length != 0)
            {
        ...
        public Object[] patch_apply(List<Patch> patches, string text)
        {
            ...
            foreach (Patch aPatch in patches)
            {
                ...
                else
                {
                    ...
                    if (end_loc == -1)
                    {
                        text2 = JavaSubstring(text, start_loc,
